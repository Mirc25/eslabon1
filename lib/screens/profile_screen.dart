...
class _ProfileScreenState extends State<ProfileScreen> {
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        if (userData!.exists) {
          ...
          // Cargar datos de países y provincias
          ...
          if (data['country'] != null && data['country']['code'] != null) {
            selectedCountry = countries.firstWhere(
              ...
              orElse: () => Country(code: '', name: '', dialCode: null), // Fallback seguro
            );
  ...
  void updateProvincesForCountry(String? countryCode) {
    setState(() {
      ...
      // Si la provincia guardada no está en la nueva lista, la reseteamos
  ...
  Future<String?> _uploadImage() async {
    ...
    try {
      final String fileName = '${currentUser!.uid}/profile_picture.jpg';
      UploadTask uploadTask = _storage.ref().child(fileName).putFile(_newProfileImage!);
      TaskSnapshot snapshot = await uploadTask;
      return await snapshot.ref.getDownloadURL();
    } catch (e) {
      print("Error uploading image: $e");
      _showErrorDialog('Error al subir la imagen de perfil.');
      return null;
    }
  }
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      await _firestore.collection('users').doc(currentUser!.uid).update({
        ...
        'profilePicture': newProfilePicUrl, // Actualiza la URL de la imagen
      });

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Perfil actualizado exitosamente!')),
      );
      // Volver a cargar los datos para reflejar los cambios, especialmente la URL de la imagen
      await _loadProfileData();
      
      // Usa context.pop() para regresar después de guardar exitosamente
      if (mounted) {
        context.pop(); 
      }
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              // Email y DNI (solo lectura)
              _buildTextField('Email', TextEditingController(text: emailDisplay), isReadOnly: true),
              _buildTextField('DNI', TextEditingController(text: dniDisplay), isReadOnly: true),
