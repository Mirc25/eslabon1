...
class _CreateRequestScreenState extends State<CreateRequestScreen> {
  ...
  Future<void> _determinePosition() async {
    ...
    try {
      ...
      // Almacenar latitud y longitud para la solicitud
      setState(() {
      ...
      debugPrint("DEBUG CREATE: Error al obtener ubicación GPS: $e");
      _showSnackBar('No se pudo obtener la ubicación actual para la solicitud.', Colors.red);
      _localityController.text = '';
      setState(() {
        _requestLatitude = null; // Asegurarse de que sean null si falla
        _requestLongitude = null; // Asegurarse de que sean null si falla
      });
    } finally {
      setState(() {
  ...
  Future<void> _getAddressFromLatLng(Position position) async {
    try {
      ...
      if (placemarks.isNotEmpty) {
        ...
        print('DEBUG CREATE: Dirección obtenida de Lat/Lng: Localidad: ${_localityController.text}, Provincia: $_userProvinceName, País: $_userCountryName');
        _showSnackBar('Ubicación precargada exitosamente.', Colors.green);
      }
    } catch (e) {
      debugPrint("DEBUG CREATE: Error al obtener dirección de lat/lng: $e");
  ...
  Future<void> _loadUserData() async {
    ...
    if (currentUser != null) {
      try {
        ...
        debugPrint("DEBUG CREATE: Error loading user data: $e");
  ...
  Future<void> _createRequest() async {
    ...
    // Validar que la ubicación se obtuvo
    if (_requestLatitude == null || _requestLongitude == null) {
      _showSnackBar('No se pudo obtener la ubicación para la solicitud. Por favor, asegúrate de tener los permisos de ubicación y conexión a internet.', Colors.red);
    ...
    try {
      ...
      await _firestore.collection('solicitudes-de-ayuda').add({
        ...
        "estado": "activa",
        "latitude": _requestLatitude,   // ¡Añadido!
        "longitude": _requestLongitude, // ¡Añadido!
      });
