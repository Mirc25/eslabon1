...
export const sendHelpNotification = onRequest(async (req, res) => {
  if (req.method !== 'POST') {
    return res.status(405).send('MÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©todo no permitido. Solo se acepta POST.');
  }
  ...
  if (!requestId || !requestTitle || !receiverId || !helperId || !helperName) {
    logger.error("Faltan parÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡metros obligatorios:", { body: req.body });
  ...
  const notification = {
    type: 'offer_received',
    title: `ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡${helperName} quiere ayudarte!`,
  ...
  try {
    ...
    logger.log("NotificaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de ayuda enviada correctamente.");
    res.status(200).send("OK");
  } catch (error) {
    logger.error("Error al guardar la notificaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n:", { error });
