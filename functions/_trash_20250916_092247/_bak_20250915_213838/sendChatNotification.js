...
export const sendChatNotification = onRequest(async (req, res) => {
  if (req.method !== 'POST') {
    return res.status(405).send('MÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©todo no permitido.');
  }
  ...
  if (!chatRoomId || !senderId || !senderName || !recipientId || !messageText) {
    ...
    return res.status(400).send("Faltan parÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡metros.");
  }
  ...
  try {
    ...
    logger.log("NotificaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de chat guardada.");
    res.status(200).send("OK");
  } catch (error) {
    logger.error("Error guardando notificaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de chat:", { error });
